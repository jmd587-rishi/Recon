...
function scriptText(params: {
  ...
  lines.push(
    ...wrapNote(
      params.writtenBy === "ai"
        ? `Written by Recon's reviewer model from the SQL in \`${folderName}\` — the table and column names below were extracted from that SQL, not invented.`
        : `Generated by Recon from the SQL in \`${folderName}\`.`,
      "   "
    )
  );
